...
function CustomInput(props: InputProps) {
    ...
    const Icon = () => {
        if(props.iconType === "font-awesome") {
            return (
                <FontAwesome5 name={props.rightIcon} size={props.iconSize ? props.iconSize : 24} color={props.iconColor ? props.iconColor : "#A9A9A9"} />
            )
        }

        return <MaterialCommunityIcons name={props.rightIcon} size={props.iconSize ? props.iconSize : 24} color={props.iconColor ? props.iconColor : "#A9A9A9"} />
    }

    return (        
        <View style={{ width: props.width }} >
            <Input placeholder={props.placeholder} blurOnSubmit={props.blurOnSubmit} disabled={props.readonly} value={props.value ? props.value.toString() : ""} label={props.label} labelStyle={props.labelStyle} secureTextEntry={props.password}
                ...
                ref={(ref) => props.setRef ? props.setRef(props.name, ref) : false} errorMessage={props.errorMessage} leftIcon={props.leftIcon ? { type: 'material-community', name: props.leftIcon, color: props.iconColor ? props.iconColor : "#A9A9A9" } : undefined}
